...
import { ResourcesClient, type ResourceView } from '@/components/resources/ResourcesClient';
...
export default async function ResourcesPage() {
  ...
  const authorIds = Array.from(new Set((resources ?? []).map((r) => r.created_by)));
  ...
  const views: ResourceView[] = ((resources ?? []) as Resource[]).map((r) => {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <ResourcesClient resources={views} meId={meId} isAdmin={isAdmin} />
      </div>
    </AppShell>
  );
}
